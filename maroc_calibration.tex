...
This calibration data was used to convert the measured amplitude in ADC channels into charge collected on an event-by-event basis. The interpolation is done by fitting quadratic functions to every calibration data point to estimate the charge as a function of ADC channels. Additionally, some linear smoothing is applied to minimize the effect of the uncertainty in the calibration data. For each event, the charge is estimated as a linearly weighted average between the extrapolations from the quadratic functions of the nearest two data points. The quadratic functions used for this interpolation are fit using the nearest 5 data points, meaning that the value of the function at the measured ADC calibration data point does not necessarily return the exact injected charge for that point. Fig.~\ref{fig:H12700calib} and Fig.~\ref{fig:H8500calib} show typical amplitude distributions before and after this conversion is applied for one H12700 MAPMT pixel and one H8500 MAPMT pixel, respectively.
